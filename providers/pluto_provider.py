...
class PlutoProvider(BaseProvider):
    ...
    def get_epg_data(self) -> Dict[str, List[Dict[str, Any]]]:
        """Get EPG data for Pluto channels"""
        try:
            token = self._get_session_token()
            if not token:
                self.logger.error("Could not get Pluto session token for EPG")
                return {}
            
            # Get current channels to get their IDs
            ...
            # Get current time for EPG
            from datetime import datetime, timezone
            
            start_time = datetime.now(timezone.utc)
            start_str = start_time.strftime("%Y-%m-%dT%H:00:00.000Z")
            ...
            for i in range(0, len(channel_ids), batch_size):
                ...
                try:
                    ...
                    epg_response = response.json()
                    
                    # Process EPG data
                    for channel_epg in epg_response.get("data", []):
                        channel_id = channel_epg.get("channelId")
                        if not channel_id:
                            continue
                        
                        programmes = []
                        for timeline in channel_epg.get("timelines", []):
                            try:
                                start_dt = datetime.fromisoformat(timeline.get("start", "").replace('Z', '+00:00'))
                                stop_dt = datetime.fromisoformat(timeline.get("stop", "").replace('Z', '+00:00'))
                                
                                title = timeline.get('title', '')
                                episode_info = timeline.get('episode', {})
                                description = episode_info.get('description', '') or episode_info.get('summary', '')
                                ...
                                programme = {
                                    ...
                                    'category': episode_info.get('genre', ''),
                                }
                                ...
                                self.logger.debug(f"Error processing programme: {e}")
                                continue
                        
                        if programmes:
                            epg_data[str(channel_id)] = programmes
                            
                except Exception as e:
                    self.logger.warning(f"Error fetching EPG batch: {e}")
                    continue
            ...
            if not epg_data:
                try:
                    ...
                    channels = self.get_channels()
                    epg_data = fallback_manager.get_fallback_epg('pluto', channels)
                    if epg_data:
                        self.logger.info(f"Using fallback EPG for pluto: {len(epg_data)} channels")
                except Exception as e:
                    self.logger.debug(f"Fallback EPG failed for pluto: {e}")

            self.logger.info(f"Retrieved EPG data for {len(epg_data)} Pluto channels")
            return epg_data
            
        except Exception as e:
            self.logger.error(f"Error fetching Pluto EPG data: {e}")
            return {}