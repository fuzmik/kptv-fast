...
class PlutoProvider(BaseProvider):
    ...
    def get_epg_data(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            channel_ids = [ch['id'] for ch in channels[:50]]  # Limit to first 50 for performance
            ...
            start_time = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:00:00.000Z")
                ...
                for timeline in channel_epg.get("timelines", []):
                    try:
                        ...
                        programme = {
                            ...
                            'description': timeline.get('episode', {}).get('description', ''),
                            'start': start_dt.strftime('%Y%m%d%H%M%S %z'),
                            'stop': stop_dt.strftime('%Y%m%d%H%M%S %z'),
                            'category': timeline.get('episode', {}).get('genre', ''),
                        }
                        ...
                        self.logger.warning(f"Error processing programme: {e}")
                        continue
